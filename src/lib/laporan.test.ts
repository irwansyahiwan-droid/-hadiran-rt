 ...
 *  2. Talangan BELUM lunas = kas keluar, dibebankan ke triwulan TARIKANNYA
 *     (bukan triwulan berjalan).
 *  3. Talangan LUNAS = uang sudah balik → net NOL, hanya menambah hitungan.
 *  4. Saldo triwulan bersifat kumulatif dari triwulan terlama.
 *  5. Rumus saldonya WAJIB sama dengan helper tunggal hitungSaldoHadiran().
 *  6. Query gagal WAJIB melempar — laporan nol yang "terlihat sah" jauh lebih
...
describe('Rekap triwulan — sumber pendapatan', () => {
  it('pendapatan hadiran dari total_terkumpul, kas_masuk manual DIABAIKAN', async () => {
    ...
    expect(q2.hadiranKeluar).toBe(0);
  });

  it('setor_kas_rt & kas_keluar dihitung sebagai keluar hadiran', async () => {
    jawab = {
      transaksi_kas: ok([
    ...
    const [q2] = await fetchRekapTriwulan();
    expect(q2.hadiranKeluar).toBe(1_400_000);
  });
});
...
describe('Rekap triwulan — talangan', () => {
  it('talangan BELUM lunas dibebankan ke triwulan TARIKANNYA, bukan triwulan berjalan', async () => {
    ...
    expect(hasil[0].hadiranKeluar).toBe(100_000);
  });
  ...
  it('talangan LUNAS net NOL — tak menambah keluar, hanya hitungan', async () => {
    ...
    expect(q1.hadiranKeluar).toBe(0);
    expect(q1.talanganLunas).toBe(1);
    expect(q1.hadiranSaldoAkhir).toBe(345_000);
  });
...
describe('Rekap triwulan — saldo kumulatif & urutan', () => {
  it('saldo triwulan berikutnya membawa sisa triwulan sebelumnya', async () => {
    ...
    expect(q1.hadiranSaldoAkhir).toBe(300_000);
    expect(q2.hadiranSaldoAkhir).toBe(300_000 + 200_000 - 100_000);   // kumulatif
    expect(q1.rtSaldoAkhir).toBe(100_000);
    expect(q2.rtSaldoAkhir).toBe(60_000);
  });

  it('saldo hadiran memakai rumus yang SAMA dengan helper tunggal', async () => {
    const kas = 345_000 * 3, talanganNyangkut = 50_000 * 2, setor = 400_000;
    jawab = {
      tarikan: ok([
        { tanggal: '2026-07-04', total_terkumpul: 345_000 },
        { tanggal: '2026-07-11', total_terkumpul: 345_000 },
        { tanggal: '2026-07-18', total_terkumpul: 345_000 },
      ]),
      transaksi_kas: ok([{ tipe: 'setor_kas_rt', nominal: setor, tanggal: '2026-07-20' }]),
      talangan: ok([
    ...
    expect(q3.hadiranSaldoAkhir).toBe(hitungSaldoHadiran(kas, talanganNyangkut, setor));
  });

  it('saldo boleh MINUS — talangan ditutup penuh dari kas, itu memang komitmennya', async () => {
    jawab = {
    ...
    const [q3] = await fetchRekapTriwulan();
    expect(q3.hadiranSaldoAkhir).toBe(-55_000);
  });
});
...
describe('Snapshot tutup buku — batas hari ini', () => {
  ...
  it('talangan nyangkut dari tarikan yang BELUM terjadi tak ikut dikurangi', async () => {
    jawab = {
    ...
    const snap = await fetchSnapshotKas();
    expect(snap.hadiranKeluar).toBe(50_000);
    expect(snap.hadiranSaldoAkhir).toBe(hitungSaldoHadiran(345_000, 50_000, 0));
  });

  it('saldo = masuk − keluar untuk kedua kas', async () => {
    jawab = {
    ...
    const snap = await fetchSnapshotKas();
    expect(snap.hadiranSaldoAkhir).toBe(45_000);
