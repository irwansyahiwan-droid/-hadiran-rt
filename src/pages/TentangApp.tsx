...
export default function TentangApp({ open, onClose }: Props) {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-[#E4EAF2] dark:bg-gray-950 page-in-right overflow-y-auto">
      <header
      ...
      <main className="max-w-lg mx-auto px-4 py-5 space-y-4" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 2rem)' }}>
        ...
        <button
          onClick={() => { haptic(); onClose(); }}
          className="press w-full min-h-[44px] py-3 rounded-2xl bg-gradient-to-r from-emerald-500 to-emerald-600 text-white font-semibold text-sm shadow-lg shadow-emerald-300/40 hover:from-emerald-600 hover:to-emerald-700 transition-all"
        >
          Tutup
        </button>
      </main>
