import { BookOpen, Coins, Users, Link2, Heart, ShieldCheck } from 'lucide-react';
...
export default function TentangApp({ open, onClose }: Props) {
  ...
  const dlg = useDialog(open, { onClose: exit.requestClose, label: 'Tentang aplikasi' });
  if (!open) return null;
    ...
    <div ref={dlg.panelRef} {...dlg.panelProps} className={`fixed inset-0 z-overlay bg-sunken dark:bg-gray-950 ${exit.closing ? 'page-out-right' : 'page-in-right'} overflow-y-auto`}>
      ...
      <main className="max-w-lg mx-auto px-4 py-4 space-y-4" style={{ paddingBottom: 'calc(env(safe-area-inset-bottom) + 2rem)' }}>
        ...
        <div className="text-center pt-2 pb-1">
          ...
          <h2 className="text-title font-bold text-gray-900 dark:text-gray-100">Hadiran RT</h2>
