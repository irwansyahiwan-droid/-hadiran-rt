  ...
  return (
    <div className={`flex items-center justify-between gap-2 mb-3 px-1 ${className}`}>
      <h2 className={`flex items-center gap-2 text-base font-bold ${tone === 'muted' ? 'text-ink-sub dark:text-gray-400' : 'text-ink dark:text-gray-100'}`}>
        ...
        {children}
        {count != null && count > 0 && (
          <span className="font-display text-[11px] font-bold text-ink-faint dark:text-gray-500 tabular-nums bg-gray-100 dark:bg-gray-800 rounded-full px-2 py-0.5">{count}</span>
        )}
      </h2>
      {action}
    </div>
