...
| `audit:sheet` | Geometri sheet/modal/popover di 360px — **plus `sheet-pendek` di 360×568** | Form di dalam bottom-sheet tak pernah diukur; kontrol bisa meluber keluar panel. **Entri TINGGI PENDEK lahir 12 Sep 2026, dan ia menutup aturan yang MUSTAHIL menggigit.** Sapuan ini punya TIGA aturan; yang ketiga ("panel lebih tinggi dari layar TANPA `overflow-y` → tombol Simpan tak tergapai") tak pernah bisa menyala di tinggi bawaan. Diukur, bukan diduga — `MUTASI=2` mencabut `overflow-y` dari TIAP dialog: **0 temuan di 360×844 · 2 di 360×568 · 4 di 360×390**. Jadi sejak aturan itu ditulis ia tak pernah sekali pun terjangkau di dalam rantai. Sebabnya sumbu yang tak pernah diukur siapa pun: sapuan repo memvariasikan LEBAR (390 12× · 360 6× · 320 2×) sementara TINGGI hardcode 844 (16×) & 800 (9×). 360×568 dipilih karena PORTRAIT & nyata (iPhone SE, Android lawas); landscape 844×390 lebih tajam tapi `manifest.orientation` = portrait sehingga cuma mengenai pemakai browser — tersedia lewat knob `H`, tidak di rantai. **Cacat probe yang ikut ketemu, dan ia kelas LAMA yang tak pernah diseberangkan:** 844×390 melaporkan 3 tombol aksi massal absensi sbg target sentuh `80×23` padahal kodenya `min-h-[44px]`. Dua tanda bahwa itu alatnya — `w:80` persis batas loop hit-test (40+40 = saturasi, bukan hasil ukur), dan permukaan yang SAMA dalam keadaan tergulir (`b9-absensi-gulir`) melapor OK. Potret sapuannya sendiri menunjukkan sebabnya: tombolnya separuh tertutup Header STICKY, jadi `elementFromPoint` dgn benar menjawab "itu header". **`audit:sentuh` sudah memperbaiki ini lebih dulu dan aturannya tertulis verbatim di dokumen ini** — perbaikannya tak pernah ikut ke sapuan kembarannya. Kini tiap kontrol di-`scrollIntoView` dulu, posisi gulir dikembalikan, dan yang TETAP tak muat dihitung `tak terukur` (diakui, bukan dilewati diam-diam). Divalidasi DUA mutasi — sapuan ini sebelumnya tak punya satu pun: `MUTASI=1` (kontrol dikecilkan 20px) **12 dari 13 permukaan MERAH**, yang membuktikan `scrollIntoView` MEMBETULKAN aturan target sentuh alih-alih menumpulkannya; `MUTASI=2` **4 dari 13 MERAH**. Garis dasar tinggi bawaan tak bergeser: 360×844 tetap 13 permukaan · 0 bermasalah |
...
| `audit:hover` | Tiap kontrol **menjawab kursor**: gaya terhitung kontrol + keturunannya (termasuk `::before/::after`) berubah saat `:hover` dipaksa lewat CDP `CSS.forcePseudoState`. Warga + bendahara, terang + gelap, 5 tab + Menu + sheet Kas RT & setor, **1280×900 tanpa sentuh** | Satu-satunya sapuan yang memakai PENUNJUK. Lahir 14 Sep 2026 dari audit Web Interface Guidelines ke-5: app ini touch-first, tapi bendahara membuka app di LAPTOP, dan **48 dari 208 kontrol unik tak berubah sama sekali** saat kursor di atasnya — bar nav, chip filter, periode grafik, Pemasukan/Pengeluaran, sub-tab Jadwal warga, baris transaksi & lipatan Beranda, kaki statistik hero, titik slide, "Proses tarikan #N", "Lihat semua" — di app yang 160 kontrol lainnya SUDAH ber-hover. Bukan "tak ada hover", tapi hover yang BOLONG, dan tak satu pun dari 30+ sapuan pernah memegang kursor. Obatnya resep, bukan salinan: `.pilihan-hover` (tinta & tepi MENGUAT, latar tak disentuh — menaikkan fill chip gelap ke gray-700 menjatuhkan label gray-400 mendekati 4,5:1; hover wajib MENAIKKAN kontras) dan `.pilihan-isi-hover` (`brightness(1.15)` untuk yang terpilih; 1.07 milik `.btn-brand` tak terlihat di fill datar hijau tua — dipotret; label putih tetap AAA 7,29–8,57). Semua digerbang `(hover: hover) and (pointer: fine)` lewat `hoverOnlyWhenSupported` — di iOS `:hover` nyangkut sesudah diketuk. **Potret menangkap regresi yang sapuannya tak bisa**: `rounded-xl` di kaki statistik hero ikut MELENGKUNGKAN garis pemisah `border-r` bahkan saat diam; lapisannya dipindah ke `::before` di belakang isi (`isolate` + `-z-10`), dan karena itu sapuan WAJIB membaca pseudo-element. **Dua penjaga populasi**: kontrol yang dua bacaan DIAM-nya sudah berbeda (autoplay, Odometer) = `tak terukur`, bukan lulus; <30% kontrol berubah = PROBE CACAT (pemaksaan tak menggigit). Nama dinormalisasi (angka → N, nominal → Rp…) supaya 13 "Proses tarikan #N" = SATU temuan. Divalidasi sebelum/sesudah (30 per tema → 0) + `MUTASI=1` (tak memaksa :hover → 274/274 PROBE CACAT). **Kontrol BARU wajib punya hover** — chip/pilihan pakai resep, baris daftar `hover:bg-gray-50 dark:hover:bg-gray-800/60` |
| `audit:umumkan` | **WCAG §4.1.3 Status Messages (AA)** — hasil PENCARIAN & CHIP FILTER diumumkan ke pembaca layar. 7 daftar: Jadwal warga, Hadiran, Kas RT, Talangan, editor Absensi, Kelola Anggota, Riwayat Aktivitas. **U1** diam saat dibuka · **U2** satu ketikan beruntun = SATU "Menampilkan N <benda>" · **U3** tanpa hasil = kalimat layar kosong · **U4** chip diumumkan | Lahir 14 Sep 2026 dari audit Web Interface Guidelines ke-6. Tujuh daftar berubah **tanpa satu suara pun**: yang melihat membaca "Tidak ada hasil", yang tidak mengetik nama warga lalu tak tahu hasilnya 12 atau nol — dan WCAG memakai "jumlah hasil pencarian" sbg contoh UTAMA §4.1.3. Tak ada sapuan yang mengetik di kolom cari lalu MENDENGARKAN. Obatnya `useUmumkanHasil(jumlah, benda, pemicu, kosong)` → `umumkanSaja()` (region polite PERMANEN Toaster, tanpa toast terlihat): hanya saat `pemicu` berubah (bukan muat/segar diam-diam), jeda 500 ms sesudah perubahan terakhir, dan kalimat kosong = judul + keterangan `EmptyState` dari konstanta YANG SAMA (pembaca layar mendengar persis yang tampil). Kata disetujui user; bendanya per halaman, Talangan = "warga" karena satu baris = satu warga (keputusan user). Urutan sengaja bukan pemicu; mengetuk status anggota di Absensi juga bukan (tombolnya membacakan statusnya sendiri). **Ikut ketemu: editor Absensi satu-satunya daftar bercari TANPA layar kosong** — cari tanpa hasil meninggalkan kartu kosong melompong; kini kanon Jadwal warga. Vonis dari region live yang direkam MutationObserver SEPANJANG jalan (pelajaran B5/B6: keadaan akhir buta urutan), dan yang TERDENGAR selalu dicetak — "0 bermasalah" tanpa isi region tak bisa dibedakan dari probe tuli. **Dua cacat alat saat membangunnya**: tombol periode grafik Kas RT ("3 bln") juga `aria-pressed` tapi tak menyaring daftar (dikecualikan), dan kolom cari Jadwal bendahara hidup di editor Absensi, bukan di tab. Divalidasi sebelum/sesudah: **17 temuan (7/7 permukaan diam) → 0**. **Daftar bercari/berfilter BARU wajib memakai `useUmumkanHasil` & masuk `PERMUKAAN`** |
| `audit:toast` | **Toast bisa DIBACA & aksinya bisa DIPAKAI sebelum pergi** — **T1** fokus di "Urungkan" menahan toast, fokus pergi melepasnya · **T2** sama untuk kursor · **T3** "Urungkan" ditekan → fokus kembali ke PEMICU, bukan `<body>` · **T4** toast yang tak dipegang hidup >= 500 ms + 300 ms/kata (200 kpm) · **T5** tumpukan toast mendahului `<header>` di DOM | Lahir 15 Sep 2026 dari audit ui-ux-pro-max (129 aturan dicocokkan lawan 40 sapuan; ini satu-satunya kelas yang tak dijaga siapa pun). `audit:tulis` & `audit:kembali` membaca toast hanya untuk tahu app MENGAKU, tak pernah apakah pengakuannya sempat terbaca atau aksinya sempat dipakai. Terukur sebelum: **semua toast 2,6 dtk** berapa pun panjangnya — 19 dari 38 pesan (17 GALAT) butuh lebih lama; aturan kata (akibat · jalan keluar · sebut bendanya) memanjangkan galat, durasinya tak ikut, jadi yang hilang justru JALAN KELUARNYA. **"Urungkan" menghitung mundur di bawah fokus & kursor** — habis waktu, aksi yang mau dibatalkan DIJALANKAN di bawah jari pengguna dan fokus jatuh ke `<body>` (§2.2.1 A · §2.4.3 A). Toaster di EKOR DOM padahal tercat paling atas: **83 Tab** dari "Semua Hadir". Obat: `durasiTampil()` di `lib/toast.ts` (lantai 2,6 dtk · galat 4 dtk · waktu baca dipagari 10 dtk · `duration` eksplisit = minimum; dikunci `toast.test.ts`, divalidasi mutasi 300 ms/kata → 0 = 4 uji merah) · jam per toast yang DIJEDA selama kursor/fokus di tumpukan & jalan lagi dgn SISA waktunya · fokus pulang ke pemicu (cadangan: fokus terakhir di luar tumpukan) · `<Toaster />` dipindah ke depan `<Header>` (3× Shift+Tab). **Dua jebakan yang menentukan desainnya:** kursor SENTUH sengaja tak menahan (`:hover` iOS nyangkut → toast tertahan selamanya), dan toast yang DICABUT di bawah kursor tak pernah mengirim `pointerleave` — pegangan diperiksa ulang lewat `:hover` sesudah DOM diperbarui. **Cacat alat saat membangunnya:** T5 meninggalkan fokus di "Urungkan" → toast (dgn BENAR) tertahan selamanya → bagian berikutnya PROBE CACAT "toast tak muncul"; kini fokus dikembalikan. Jarak Shift+Tab dicetak tapi TAK divonis — Chromium headless membungkus Shift+Tab dari awal dokumen ke ekornya, jadi build lama pun terbaca 4×; yang divonis urutan DOM. Divalidasi sebelum/sesudah (**5 dari 6 merah → 8 · 0**) + `MUTASI=1` (toast tak dipegang → T1 & T2 merah). **Toast beraksi BARU wajib lewat `showUndo`/`showToast` — jangan pasang pewaktu sendiri** |
| `audit:tautan` | **Tautan per tab** (`/jadwal` · `/hadiran` · `/kas-rt` · `/talangan`, Beranda `/`) + "Bagikan halaman ini" di Menu. T1 tautan langsung (5 tab) · T2 warga lewat LOGIN mendarat di tab tautan · T3 nav → alamat · **T4 balapan** dari tautan → Jadwal → Beranda satu task · T5 Back → Beranda "/" lalu keluar app · **T6 Back di Login bertautan tak hangus** · T7 Back menutup sheet saja · T8 path asing → "/" · T9 keluar warga → "/" · T10 bagikan (teks+URL · salin+toast · batal = diam) · **T11 reload** (sesudah nav & dgn sheet terbuka) | Lahir 14 Sep 2026 bersama fiturnya (keputusan user: path pendek, item Menu, kata disetujui). Tautan yang dibagikan ke grup WA dulu SELALU mendarat di Beranda. Fitur ini menempel di DUA mesin yang pernah membuat app blank/tuli — back-stack `useBackDismiss` & gate Login — dan cacatnya tak terlihat dari layar: Beranda yang beralamat `/kas-rt` terlihat sehat sampai tautannya disalin. **Tiga cacat nyata ditangkap SAAT membangunnya, semuanya di kode fitur ini sendiri:** (1) alamat diganti `replaceState` langsung kalah balapan dgn `history.back()` asinkron milik cleanup entri tab → Beranda beralamat tab lama; obatnya `gantiPath()` lewat antrean `runOrQueue` yang sama, dipanggil dari EFEK (dari handler ia jalan SEBELUM cleanup sempat mengantre — T9 merah persis begitu). (2) Entri Back tab terdaftar di bawah layar LOGIN bertautan → ketukan Back pertama hangus (kelas bagian D `audit:mundur`); kini digerbang `cangkangTampil`. (3) **Reload**: penyapu entri yatim mundur ke entri di bawah yang beralamat "/", App membaca alamat sesudahnya → tab hilang; tertangkap `audit:masuk` ("reload tak memulihkan tab aktif") begitu kunci sessionStorage `hadiran-tab-sesi` dilepas — kini penyapu memulihkan `alamatSaatBoot()` sebelum antrean dilepas & App membaca tab awal dari sana. Divalidasi MUTASI + dua sebelum/sesudah: `MUTASI=1` (replaceState dibisukan) → T3/T4/T5/T8/T9 merah; `gantiPath` tanpa antrean → **T4 ×2 & T9 merah "alamat /kas-rt"**; penyapu tanpa pemulihan → **T11 merah "tab Beranda · alamat /"**. Slug = JANJI PUBLIK (tautan lama di chat) — dijaga `tautanTab.test.ts` (bolak-balik tiap tab, unik, tak bentrok rewrite publik `vercel.json`). **Tab baru wajib punya slug; alamat HANYA diganti lewat `gantiPath()`** **T12 (15 Sep 2026, audit Web Interface Guidelines ke-7)**: sejak tab punya alamat, navigasi di KONTEN yang masih `<button onClick>` melanggar "`<a>` untuk navigasi" — tekan-lama Android tak menawarkan "Salin alamat link", Ctrl/⌘-klik bendahara tak membuka tab baru. Delapan navigasi (Lihat semua ×3, banner talangan, kaki hero ×3, CTA carousel) kini `<a href>` + `klikTautanSpa()` (klik biasa pindah tab di tempat, bermodifier dibiarkan ke peramban; `draggable={false}` di dalam carousel supaya seretan native tak merebut seretan kartu). T12 memeriksa href, klik biasa TANPA muat ulang, dan Ctrl-klik → tab peramban baru sementara halaman ini diam. Sebelum **5 merah** (belum ada satu `<a>` pun di app) → 0; Beranda **identik per piksel** di terang & gelap. **Bar nav bawah SENGAJA tetap `<button>`**: 31 sapuan memilih tab lewat `nav button`, 14 di antaranya dikunci read-only — mengubahnya berarti membuka kunci itu (keputusan user). **Navigasi BARU ke tab wajib `<a href={pathTab(…)}>` + `klikTautanSpa`** |
| `audit:fokus-tertutup` | **§2.4.11 Focus Not Obscured (AA)** + §2.4.12 (AAA, ambang app): elemen yang difokus papan ketik TERTUTUP chrome sticky/fixed milik app — Tab maju **dan** Shift+Tab mundur, warga + bendahara, 5 tab + 5 overlay, **390×844 & 360×568** | `audit:papan-ketik` bertanya apakah kontrol TERGAPAI Tab; tak satu pun sapuan bertanya apakah yang tergapai itu TERLIHAT. Lahir 13 Sep 2026 dari audit Web Interface Guidelines, bukan dari rantai. Peramban meratakan elemen fokus ke TEPI viewport — tepi yang diduduki Header sticky (atas) serta bar nav & FAB fixed (bawah); tanpa `scroll-padding` fokus mendarat tepat di bawahnya. **Garis dasar pertama @390×844: 1042 titik · 34 tertutup PENUH · 333 sebagian · 65 DI LUAR LAYAR**, tiga kelas: Header menutup yang dicapai mundur ("Apa itu Sohibul Bait?" 0%, 16 tombol aksi tarikan Jadwal 0%), bar nav menutup yang dicapai maju ("Urutkan", chip filter Hadiran 0%), dan **bar nav TUCKED tetap memegang tombolnya di urutan Tab** — fokus di y=852 pada layar 844, kedua peran (kembaran persis cacat FAB 19 Agu, yang obatnya tak pernah diseberangkan ke nav). Sisa sesudah dua obat pertama: FAB menutup 36–80% baris (63 baris Kelola Anggota — di overlay FAB TAK PERNAH menyingkir karena `useScrollHide` mendengar gulir `window`). Obatnya `useCadanganGulir(ref, sisi)` dipasang di KOMPONEN chrome (Header, OverlayHeader, BottomNav, Fab), bukan per halaman — overlay baru ikut aman. **Tiga keputusan, tiap satu jebakan terukur:** tinggi diukur dari ELEMENNYA (Header menyusut saat gulir & tumbuh saat strip LURING/BASI); cadangan = offset CSS + `offsetHeight`, BUKAN `getBoundingClientRect` (nav/FAB menyingkir lewat transform → rect tucked di luar layar → cadangan nol yang tak pernah pulih karena transform tak memicu ResizeObserver); beberapa chrome di satu wadah digabung MAKS per sisi (nav & FAB sama-sama menulis sisi bawah — percobaan pertama menimpa). Plus nav: **fokus memunculkannya** (pola FAB). Sesudahnya **2078 titik · 32 layar · 0 · 0 · 0**. Vonis dari HIT-TEST `elementFromPoint` kisi 5×5, dan hanya penutup fixed/sticky yang dihitung (tumpang-tindih biasa bukan kelas ini). Divalidasi DUA mutasi karena DUA cabang: `MUTASI=1` (cabut semua scroll-padding) → **penuh 34 · sebagian 333, PERSIS angka pra-perbaikan**; `MUTASI=2` (nav dipaksa tucked) → **82 DI LUAR LAYAR**. Mutasi wajib membuktikan ia MENDARAT (penghitung per layar). Layar tanpa chrome sticky/fixed = PROBE CACAT (tak ada yang bisa menutupi → probe tak bisa menggigit). **Chrome sticky/fixed BARU wajib memakai `useCadanganGulir`.** `window.scrollTo` tak terpengaruh `scroll-padding` — hanya fokus & `scrollIntoView` |
...
| `audit:muat` | **HARGA FONT di jalur kritis** — selisih dua lengan se-jalan (dgn font vs seluruh woff2 ditolak), plus FCP & siap-pakai mutlak yang DILAPORKAN tapi tak pernah jadi vonis (CPU 4× lambat, 400 kbps) | Warga pakai Android kelas bawah, sinyal seadanya — tapi sampai 12 Sep 2026 sapuan ini **meteran, bukan penjaga**: 75 baris tanpa satu pun baris vonis, selalu exit 0, dan tak terdaftar di `sapu-semua`. Padahal app ini MEMBELI waktu muat sadar-sadar (`900e80e` memilih varian X seharga 531 ms untuk menutup kedip huruf 843 ms), dan tak ada satu alat pun yang akan berubah merah kalau harga itu berlipat — pelajaran ke-33 persis. **Angka MUTLAK tak bisa jadi ambang**: kode yang PERSIS SAMA membaca siap-pakai 3291 ms di satu mesin & 3543 ms di mesin lain, jadi ambang mutlak akan merah karena mesinnya. Yang machine-independent adalah SELISIH dua lengan yang diukur di JALAN YANG SAMA — invarian "dokumen dibandingkan dgn DIRINYA SENDIRI" milik penjaga kertas, dipindah ke sumbu waktu — dan ia bahkan punya ramalan FISIK: `byteFont ÷ (KBPS ÷ 8)` = 725 ms, terukur 697–725 ms lintas build & mesin. Anggaran **1400 ms** (FCP 160) — dinaikkan dari 900 pada 12 Sep 2026 sbg PEMBELIAN YANG DISETUJUI, sesudah EMPAT jalan tengah diukur & tiga terbukti buntu (`fetchpriority=low` hemat 9 ms · preload sesudah skrip modul 38 ms · sumbu bobot 600:800 13,5 ms / 700:800 38 ms · subset karakter 231 ms tapi DITOLAK karena menukar 231 ms sekali-jalan dgn melemahkan penjaga "nol glyph hilang" permanen + risiko berbentuk DATA pada nama warga). Prioritas & urutan tak bisa menolong karena di 400 kbps yang mahal BYTE-nya, bukan antreannya — pipanya DIBAGI bukan berurutan, terbukti dari Sora 25 kB yang selesai SEBELUM Inter 37 kB di ketiga varian bahkan saat Sora diprioritaskan `Low`. **Kalau preload Sora suatu saat dilepas, TURUNKAN kembali ke 900** — plafon yang tertinggal tinggi sesudah sebabnya hilang tak menjaga apa pun. Anggaran **JANGAN diturunkan dari ukuran font**: plafon yang ikut tumbuh bersama benda yang dijaganya cuma mencatat. Yang TIDAK dijaga di sini & jangan dikira begitu: waktu muat mutlak, anggaran BYTE (`audit:unduh` U), kedip huruf (`audit:unduh` F & `audit:fallback-sora`). Divalidasi DUA mutasi: `MUTASI=1` menyajikan SATU FONT LAGI di jalur kritis lewat URL SINTETIS → MERAH 1238 → **1787 ms**; `MUTASI=2` blokir pura-pura → PROBE CACAT (tanpa itu selisih −29 ms terbaca hijau PALSU). **URL sintetisnya wajib BARU, dan ini pelajaran tersendiri:** versi pertama mem-preload ULANG Sora, dan sejak `ed8b1d6` Sora sudah di-preload sehingga peramban men-DEDUPE-nya — mutasi itu mencetak +1252 ms, angka yang SAMA PERSIS dgn jalan tanpa mutasi, dan hanya TAMPAK merah karena garis dasarnya kebetulan sudah di atas anggaran. Ia akan berubah hijau diam-diam begitu anggarannya dinaikkan. **Mutasi yang menduplikasi sumber daya yang sudah ada bukan mutasi — ia dedupe.** **TIGA cacat probe dibayar saat membangunnya**: (a) matcher rute membandingkan href utuh sehingga `http://host:5199/` tak pernah cocok dgn `APP` — mutasi diam-diam tak terjadi & sapuan tetap hijau, jadi mutasi kini WAJIB punya penghitung yang membuktikan ia mendarat; (b) **`route.fulfill` Playwright TIDAK melewati emulasi jaringan CDP** — dokumen tiba GRATIS & FCP jatuh ke 140 ms padahal latensi emulasi 400 ms, angka MUSTAHIL yang jadi vonisnya sendiri (pelajaran ke-7), jadi mutasi TIMING wajib hidup di lapisan SERVER (sajikan `dist/` dari server sendiri, preseden `audit:unduh`) bukan intersepsi; (c) **`document.fonts` itu BALAPAN, bukan properti** — uji kontrol yang menuntut face ber-status `loaded` saat tombol muncul LOLOS di lokal tapi PROBE CACAT di produksi (CSS non-blocking 86 kB baru terparse sesudah tombolnya ada → nol `@font-face` terdaftar), dan PROBE CACAT itu muncul justru pada jalan yang sedang MENGANDUNG temuan nyata; bukti yang benar = berapa woff2 yang benar-benar DITERIMA tiap lengan. **Temuan pertamanya DIPUTUSKAN 12 Sep 2026** (keputusan user, sesudah empat lever diukur): preload Sora DIPERTAHANKAN & anggaran dinaikkan sadar-sadar, karena ongkosnya dibayar SEKALI seumur pemasangan — terukur kunjungan 1 = 3997 ms, kunjungan 2 & 3 = **921 ms** dgn judul tetap Sora — sementara yang dibeli permanen. `main` merah +1253 ms (produksi brotli +1094) karena `ed8b1d6` mem-preload Sora demi menghentikan judul dicat Inter. Commit itu menulis "ongkos nol byte" — benar untuk BYTE, salah untuk WAKTU: di kabel yang jenuh, 25 kB yang pindah ke depan antrean membuat chunk entry & vendor-react menunggu di belakangnya. **"Nol byte" bukan "nol ongkos"; antrean punya harganya sendiri.** |
...
| `audit:fallback-sora` | **A.** teks yang MUAT saat Sora ada tapi TERPOTONG saat Sora tidak ada · **B.** teks Sora yang BERTAMBAH BARIS saat jatuh ke Inter. Warga + SELURUH permukaan bendahara & sheet, 320 + 390px | Sejak 5 Sep 2026 Sora dipasang `font-display: optional` — itu yang menutup kedip 1.898 ms, tapi konsekuensinya di jaringan lambat SELURUH judul & wordmark dirender **Inter**, dan Inter **8,7% LEBIH LEBAR** (238,3 → 259 px pada 700/32px). Arah yang berisiko luber. **Tak satu pun dari 35 sapuan melihat keadaan itu, dan sebabnya struktural:** semuanya berjalan di localhost, tempat Sora selalu MENANG balapan 100 ms — keadaan yang cuma muncul di jaringan lambat karena itu tak pernah masuk populasi siapa pun. Waktu `optional` dipilih, keadaan ini diukur MANUAL sekali (3 lebar × 4 tab WARGA) dan batasnya diakui terang-terangan: bendahara & sheet belum diukur. Sapuan ini menutup batas itu dan menjadikannya penjaga alih-alih catatan. **Populasi A = SELURUH teks terkurung, bukan hanya teks bertumpuk Sora** — memblokir Sora menggeser tata letak, jadi tetangganya bisa ikut terdorong; membatasi populasi ke tumpukan Sora justru melewatkan efek lanjutannya. **Bagian B semula memvonis dari MARGIN dan itu TEMUAN PALSU:** ia melaporkan 13 "tak muat", di antaranya "STRUKTUR PENGURUS RT 004/006" kurang 58px — kotaknya memang lebih sempit dari teks satu baris, tapi teksnya MELIPAT, dan judul yang melipat tidak rusak. Vonis margin hanya sah untuk wadah yang tak bisa melipat, dan wadah semacam itu sudah dijaga bagian A. Kini B memvonis dari `getClientRects().length` pada keadaan yang BENAR-BENAR dirender di tiap pass. **KONTROL-nya yang paling penting di sini:** probe pertamaku memblokir font TANPA PERNAH MENGGIGIT dan tetap mencetak "0px selisih" — hijau yang berasal dari blokir yang tak terjadi; tiap jalan kini WAJIB membuktikan berapa woff2 ditolak DAN `document.fonts.check` true→false. **`DILEWAT` = PROBE CACAT, bukan catatan** (pelajaran `audit:mundur`): target Kas RT & revisi jadwal hanya ADA kalau DB-nya berisi, jadi lingkungan tanpa Supabase menstarve populasi — dan luapan teks bergantung pada STRING NYATA (nama warga, keterangan), sehingga hijau dari data palsu tak mengatakan apa pun tentang produksi. Divalidasi MUTASI (`MUTASI=1` melebarkan teks di pass fallback → **A 0→13, B 0→14**, masing-masing menyebut teksnya). Garis dasar LAWAN DB HIDUP (5 Sep 2026): **40 permukaan · 1032 teks terkurung · 568 teks Sora · A 0 · B 0**, nol `dilewat`. Di lingkungan tanpa DB ia menyusut ke 30 permukaan / 64 teks dan keluar `PROBE CACAT` — selisih 16x itu sendiri bukti kenapa gerbang `dilewat` ada. **Preview yang mati dijawab PROBE CACAT dua baris, bukan jejak tumpukan `ERR_CONNECTION_REFUSED`** — sapuan yang gagal dgn cara tak terbaca membuat orang menduga APP-nya yang rusak |
...
| `audit:jaga-isian` | **A.** Form yang sudah diisi tak terbuang tanpa tanya — 7 form bendahara (tambah transaksi Kas RT, setor Kas Hadiran, target Kas RT, tambah & revisi jadwal, tambah anggota, editor Absensi) × SEMUA jalur tutup: Back HP, Back LAGI sesudah "Lanjut mengisi", ketuk latar, seret, Escape, tombol merah, lalu invarian back-stack. **B.** Simpan pada form kosong menjelaskan: pesan di bawah kolom, fokus pindah, `aria-describedby`, NOL permintaan tulis | Lahir 13 Sep 2026 dari audit Web Interface Guidelines ("warn before navigation with unsaved changes"). Tujuh form membuang isian lewat Back/latar/seret tanpa satu pertanyaan; taruhan tertingginya editor **Absensi** — Back di tengah menandai puluhan anggota membuang semuanya. Kata-katanya disetujui user lebih dulu. Obatnya `useJagaIsian(berubah, tutup)` + `ConfirmDestruktif` ber-`batalLabel="Lanjut mengisi"`; isian yang belum disentuh tetap tertutup LANGSUNG (A1 — penjaga yang bertanya tiap kali dilatih untuk diabaikan). **Jebakan yang menentukan desainnya: Back HP memakan entri history SEBELUM `close()` dipanggil.** Sheet yang bertanya lalu tetap terbuka tak lagi terdaftar, dan Back berikutnya pindah tab (form ter-unmount) atau keluar app — jadi `backAktif` dimatikan selama dialog bertanya & dinyalakan lagi saat "Lanjut mengisi". `ConfirmDestruktif`-nya WAJIB SAUDARA wadah sheet, bukan anaknya: klik di dialog menggelembung ke `onClick` wadah dan "Lanjut mengisi" langsung membukanya lagi. Seretan ditahan lewat opsi baru `useDragDismiss(onClose, { cegahTutup })`. **Galat inline** lewat `useGalatKolom` + `GalatKolom`; form WAJIB `noValidate` — `required` bawaan memunculkan gelembung berbahasa INGGRIS dan menahan submit sebelum handler app jalan, jadi galat inline tak pernah tampil. Warnanya pasangan toast galat (`text-neg`/`rose-400`) karena permukaan ini hanya lahir saat galat dan tak pernah dirender sapuan kontras mana pun. **Divalidasi TIGA arah**, tanpa flag MUTASI (mutasi bermakna ada di kode app): build SEBELUM penjaga → A2 merah 7/7; mutasi sumber `backAktif: !membuang` + `GalatKolom` bisu → **A4 merah 7/7 dgn "tab Kas RT → Beranda, dialog 1 → 0"** & B1 6/6; mutasi `useDragDismiss` abaikan `cegahTutup` + validasi Kas RT dicabut → A6 6/6 & B1–B4 (termasuk "MENGIRIM 1 permintaan tulis"). **Tiga cacat alat dibayar saat validasi:** (a) sapuan MATI di bagian B karena desync melempar app keluar — kini tiap form terisolasi try/catch jadi PROBE CACAT bernama (pelajaran ke-20); (b) **riak berantai**: satu cacat melahirkan 38 temuan (A5–A9 memvonis form yang sudah lenyap) — kini sesudah TIAP langkah A, form lenyap = sisa A berhenti & diakui, satu cacat satu baris; (c) pemulihan bagian B mencoba mengetuk Menu yang masih ditutupi overlay — kini mulai dari app segar kalau ada lapisan terbuka. **Form tulis BARU wajib memakai `useJagaIsian` + `useGalatKolom` dan masuk daftar `FORM` sapuan ini.** **14 Sep 2026 — dua tambahan.** (1) Mode EDIT transaksi Kas RT & edit anggota MASUK populasi (dulu diakui sbg batas "komponen sama, kata beda" — padahal kata yang beda itu persis yang tak pernah terukur); judul "Buang perubahan…" & tombol "Simpan Perubahan" hanya ada di mode edit, jadi pemicu yang meleset ke mode tambah pasti merah. (2) **Bagian C: HAPUS TARGET Kas RT** — tombolnya dulu LANGSUNG menghapus, satu-satunya aksi merusak di app yang lolos dari kanon `ConfirmDestruktif`. Kini bertanya (kata disetujui user); C1 wajib dialog & nol tulis, C2 Back menutup DIALOG saja, C3 Batal kembali ke sheet utuh — tombol merah SENGAJA tak pernah diketuk (CLAUDE.md melarang Hapus di data produksi, jadi validasi "sebelum" WAJIB lokal: build tanpa konfirmasi → **C1 merah "dialog: 0, tulis: 1"**). Tinggal di sini karena `audit:mundur` dikunci read-only. C2–C3 dilewati kalau C1 merah (disiplin riak berantai). Knob `HANYA=` untuk validasi, populasi saringannya dicetak. **9 form · 113 pemeriksaan · 0.** **B5 & B6 (14 Sep 2026, audit Web Interface Guidelines kedua) — galat inline buatanku sendiri ternyata BISU bagi pembaca layar, dan bagian B yang hijau tak bisa melihatnya karena ia hanya memeriksa keadaan SESUDAH.** `tampilkan()` memanggil `focus()` sinkron sesudah `setGalat`, jadi fokus MENDARAT sebelum React merender atributnya — terukur lewat `focusin`: `describedby: null, invalid: null`; dan Enter di kolom kosong yang SUDAH difokus memunculkan galat dgn NOL peristiwa fokus. B1–B3 hijau di keduanya karena atributnya memang ADA — cuma terlambat satu render. **Sapuan yang memotret keadaan akhir buta terhadap URUTAN; aksesibilitas yang dibacakan pada peristiwa wajib diukur PADA peristiwanya.** Obat: fokus dipindah di efek SESUDAH render; kalau kolomnya sudah difokus, pesan diumumkan lewat `umumkanSaja()` (region live `polite` PERMANEN milik Toaster, tanpa toast terlihat). Tutup-lalu-fokus-ulang SENGAJA ditolak: di HP itu menutup & membuka papan ketik. Divalidasi sebelum/sesudah: build lama → B5 merah di tiap form, B6 merah di form ber-`<form>` ("galat tampil tapi region polite berbunyi "" (peristiwa fokus: 0)"). **Cacat alat saat membangunnya:** percobaan pertama B6 mengosongkan kolom lewat kondisi terbalik, kolomnya berisi "1", dan Enter memvonis kolom BERIKUTNYA — terbaca dari "peristiwa fokus: 1" yang mustahil untuk kolom yang sudah difokus. |
...
**Lantai populasi di `sapu-semua` (3 Sep 2026).** Rantai ini pernah mencetak **24 hijau** dari jalan yang diam-diam mengukur SEPARUH populasinya — `sentuh` 410 → 360 kontrol, `sheet` 13 → 7 permukaan — dan tiap sapuan tetap keluar 0 karena memang tak ada temuan pada apa pun yang sempat diukur. Diperiksa ulang satu per satu, keduanya pulih; itu flake mesin, dan justru itu masalahnya: tak ada yang memberi tahu pembacanya bahwa laporan itu berdiri di atas populasi separuh. Cacat ke-23 dulu menutup populasi KOSONG (`PROBE CACAT`); populasi SEPARUH lolos sampai hari ini. Kini tiap sapuan punya pola + lantai (~95% garis dasar terukur) dan turun di bawahnya = MERAH. **Ketat, bukan longgar** — populasi app ini stabil antar-jalan (`sentuh` 410/412/410, `sheet` 13 selalu), jadi toleransi 15% justru meloloskan penurunan 12% yang memicu penjaga ini dibuat; kalau data memang bertambah, PERBARUI lantainya. **Pola yang tak cocok juga MERAH** (`POLA POPULASI HILANG`): keluaran sapuan yang berubah bentuk membuat penjaga ini buta, dan penjaga buta yang diam persis kelas cacat yang mau ditutup. Tiga sapuan sempat TANPA lantai — `lebar`, `reflow`, `gerak` — karena keluarannya tak menyebut satu pun angka populasi; **ditutup di hari yang sama** dgn menambahkan penghitung ke masing-masing (`118 konteks diperiksa` · `10 layar diperiksa` · `14 tab diperiksa`). Daftar `TANPA_LANTAI` sengaja DIPERTAHANKAN meski kosong: sapuan BARU yang belum mencetak populasinya ditaruh di sana supaya celahnya tercetak tiap jalan, bukan jadi catatan yang nyaman. **Sapuan yang tak mencetak berapa yang diukurnya TIDAK BISA dijaga** — ia boleh mengukur separuh lalu tetap melapor 0 temuan, dan itu persis bentuk cacat yang lantai ini dibuat untuk menangkap. Divalidasi MUTASI (`MUTASI=1` menaikkan tiap lantai 10× → semua sapuan berlantai WAJIB melapor POPULASI TURUN; terbukti 5/5 di jalur statis).
...
**DITUTUP di hari yang sama oleh `audit:fallback-sora`.** Catatan aslinya
berbunyi: dgn Sora `optional`, di jaringan lambat kunjungan PERTAMA judul
dirender **Inter**, yang **8,7% LEBIH LEBAR** (238,3 → 259 px pada 700/32px);
diukur manual sekali di 4 tab warga, dan **permukaan bendahara & sheet belum
diukur**. Kini ia punya alat: 30 permukaan (warga + SELURUH permukaan bendahara,
FAB, overlay admin & sheet) di 320+390px, **A 0 luapan baru · B 0 bertambah
baris**, divalidasi mutasi (A 0→13, B 0→14).

**DIJALANKAN LAWAN DB HIDUP 5 Sep 2026 — dan selisihnya membenarkan sapuan itu
menolak menyebut jalan pertamanya hijau:**

    lingkungan tanpa DB   30 permukaan ·   64 teks terkurung ·  64 teks Sora
    lawan DB hidup        40 permukaan · 1032 teks terkurung · 568 teks Sora

Populasi teksnya **16x lebih besar**, dan sisi bendahara naik 11 -> 16 permukaan
per lebar (target Kas RT, menu aksi tarikan & revisi jadwal cuma ADA kalau
datanya ada). **Hasilnya A 0 · B 0 di keempat sel** — 320/warga 191 teks,
320/bendahara 319, 390/warga 197, 390/bendahara 325, nol `PROBE CACAT`.
Keadaan judul-ber-Inter karena itu TIDAK merusak apa pun di permukaan mana pun,
dan sekarang ada alat yang mencetaknya tiap jalan.

**Lantai populasinya baru bernyawa sesudah jalan ini**: 36 (ditebak dari
pencacahan kode) -> 38 (~95% dari 40 yang terukur). Lantai yang ditebak sebelum
ada garis dasar nyata itu tebakan, bukan penjaga — kelas yang sama dgn `garis:
47` yang pernah nyaris meloloskan penyusutan populasi 74%.

Pelajaran alat yang lahir saat membangunnya, dan ini yang ketiga kalinya di
sesi yang sama: **vonis awalku memakai proksi yang terdengar masuk akal tapi
salah.** Bagian B semula memvonis dari MARGIN — lebar teks kalau dirender Inter
lawan lebar kotaknya — dan melaporkan 13 "tak muat", terburuk "STRUKTUR
PENGURUS RT 004/006" kurang 58px. Semuanya PALSU: kotaknya memang lebih sempit
dari teks satu baris, tapi teksnya **MELIPAT**, dan judul yang melipat tidak
rusak. Vonis margin cuma sah untuk wadah yang tak bisa melipat — dan wadah
semacam itu justru sudah dijaga bagian A. Yang benar-benar bisa dilihat warga
...
Yang ke-43 (13 Sep 2026) — **perbaikan aksesibilitas yang benar memusnahkan
populasi sapuan kontras, dan sapuan itu tetap hijau.**

Chrome membacakan tiap ikon lucide polos sbg `image` tanpa nama (76 di
Hadiran). Plugin `ikonDekoratif` (vite.config.ts) memasang `aria-hidden` bawaan
di semua ikon tanpa nama — benar untuk pohon aksesibilitas. Tapi
`audit:kontras-nonteks` memakai `svg.closest('[aria-hidden]')` sbg tanda
"dekoratif", dan **`closest` MENYERTAKAN elemen itu sendiri**. Diukur dgn aturan
lama di build baru: **ikon 499 → 0 sampel, TOTAL 751 → 252, keluar 0 "0 gagal"**
— seluruh bagian A lenyap, termasuk ikon tombol ikon-saja yang satu-satunya
label visualnya. Lantai `sapu-semua` (700) memang akan memerahkannya, tapi
jalan sendirian tidak. Kini saringannya LELUHUR saja: ikon 509 (+10 ikon ber-
`aria-hidden` eksplisit yang ternyata buta sejak dulu), bagian lain identik.

**Pelajaran: sebelum mengubah atribut yang dipakai sapuan sbg PROKSI populasi,
ukur populasi sapuan itu SEBELUM & SESUDAH.** Proksi (`aria-hidden` = dekoratif)
yang benar hari ini berubah jadi salah begitu artinya digeser di tempat lain.

Sekalian ketahuan: **`sw-manifest` menimpa galat build plugin lain.** Rollup
memanggil `closeBundle` JUGA saat build gagal; plugin itu lalu meledak sendiri
("kait VERSI tak ada") dan galat asli (`ikon-dekoratif: kait … tak ketemu`)
tak pernah tercetak. Validasi mutasi pertamaku nyaris membacanya sbg "gerbang
ledak plugin tak bekerja". Kelas pelajaran ke-40 persis.
