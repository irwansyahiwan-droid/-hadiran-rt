...
| `audit:muat` | **HARGA FONT di jalur kritis** — selisih dua lengan se-jalan (dgn font vs seluruh woff2 ditolak), plus FCP & siap-pakai mutlak yang DILAPORKAN tapi tak pernah jadi vonis (CPU 4× lambat, 400 kbps) | Warga pakai Android kelas bawah, sinyal seadanya — tapi sampai 12 Sep 2026 sapuan ini **meteran, bukan penjaga**: 75 baris tanpa satu pun baris vonis, selalu exit 0, dan tak terdaftar di `sapu-semua`. Padahal app ini MEMBELI waktu muat sadar-sadar (`900e80e` memilih varian X seharga 531 ms untuk menutup kedip huruf 843 ms), dan tak ada satu alat pun yang akan berubah merah kalau harga itu berlipat — pelajaran ke-33 persis. **Angka MUTLAK tak bisa jadi ambang**: kode yang PERSIS SAMA membaca siap-pakai 3291 ms di satu mesin & 3543 ms di mesin lain, jadi ambang mutlak akan merah karena mesinnya. Yang machine-independent adalah SELISIH dua lengan yang diukur di JALAN YANG SAMA — invarian "dokumen dibandingkan dgn DIRINYA SENDIRI" milik penjaga kertas, dipindah ke sumbu waktu — dan ia bahkan punya ramalan FISIK: `byteFont ÷ (KBPS ÷ 8)` = 725 ms, terukur 697–725 ms lintas build & mesin. Anggaran 900 ms (FCP 160) **JANGAN diturunkan dari ukuran font**: plafon yang ikut tumbuh bersama benda yang dijaganya cuma mencatat. Yang TIDAK dijaga di sini & jangan dikira begitu: waktu muat mutlak, anggaran BYTE (`audit:unduh` U), kedip huruf (`audit:unduh` F & `audit:fallback-sora`). Divalidasi DUA mutasi: `MUTASI=1` preload font kedua → MERAH +1248 ms; `MUTASI=2` blokir pura-pura → PROBE CACAT (tanpa itu selisih −29 ms terbaca hijau PALSU). **TIGA cacat probe dibayar saat membangunnya**: (a) matcher rute membandingkan href utuh sehingga `http://host:5199/` tak pernah cocok dgn `APP` — mutasi diam-diam tak terjadi & sapuan tetap hijau, jadi mutasi kini WAJIB punya penghitung yang membuktikan ia mendarat; (b) **`route.fulfill` Playwright TIDAK melewati emulasi jaringan CDP** — dokumen tiba GRATIS & FCP jatuh ke 140 ms padahal latensi emulasi 400 ms, angka MUSTAHIL yang jadi vonisnya sendiri (pelajaran ke-7), jadi mutasi TIMING wajib hidup di lapisan SERVER (sajikan `dist/` dari server sendiri, preseden `audit:unduh`) bukan intersepsi; (c) **`document.fonts` itu BALAPAN, bukan properti** — uji kontrol yang menuntut face ber-status `loaded` saat tombol muncul LOLOS di lokal tapi PROBE CACAT di produksi (CSS non-blocking 86 kB baru terparse sesudah tombolnya ada → nol `@font-face` terdaftar), dan PROBE CACAT itu muncul justru pada jalan yang sedang MENGANDUNG temuan nyata; bukti yang benar = berapa woff2 yang benar-benar DITERIMA tiap lengan. **Temuan pertamanya masih TERBUKA**: `main` merah +1253 ms (produksi brotli +1094) karena `ed8b1d6` mem-preload Sora demi menghentikan judul dicat Inter. Commit itu menulis "ongkos nol byte" — benar untuk BYTE, salah untuk WAKTU: di kabel yang jenuh, 25 kB yang pindah ke depan antrean membuat chunk entry & vendor-react menunggu di belakangnya. **"Nol byte" bukan "nol ongkos"; antrean punya harganya sendiri.** |
